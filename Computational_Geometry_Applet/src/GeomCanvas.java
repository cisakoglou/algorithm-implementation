...
public class GeomCanvas extends Canvas
{
  ...
  private ConvexHull2Dorth myCHorth;
  ...
  private boolean first, first2, done, done1, done2, toClear, toDelete, 
                  ...
                  toPaint, toDelTri, firstpoly, secndpoly, toCH, toCHorth;
  ...
  GeomCanvas(int cw, int ch, CompGeomTest frame) { 
	...
	myCHorth = new ConvexHull2Dorth(list);
  ...
  public void paint(Graphics g)
  {
	...
	IsNotDone();
      ...
      if (regime.equals(POLY) || regime.equals(ORTH))     // polygon is the input
      {
	...
	if (segmts.code != '0')
	  ...
	  if (regime.equals(ORTH)){
		if (toCHorth){
			myCHorth.RunHull();
			myCHorth.DrawHull(gContext, w, h);
			toCHorth = false;
		}
	  }
  ...
  public boolean mouseDown(Event e, int x, int y)
  {
    ...
    if (regime.equals(ORTH) && listCur.n > 0){
	cVertex last = listCur.head;
	if (xset <= last.v.x+(w/2) && xset >= last.v.x-(w/2) 
	    && yset <= last.v.y+(h/2) && yset >= last.v.y-(h/2)){
	    xset = last.v.x; yset = last.v.y;
	}
	else {
	    cPointi l = listCur.head.prev.v;
	    int dx = 0, dy = 0;
    ...
    if(!donex && !(toAdd || toMove || toDelete || aToMove))
    {
	  ...
	  if (listCur.n != 0) {
		...
		if (lastpoint && (regime.equals(ORTH)) && (last.v.x != last.prev.v.x) && (last.v.y != last.prev.v.y)){
			if (Math.abs(last.v.x - last.prev.v.x) < Math.abs(last.v.y - last.prev.v.y)){
				listCur.ResetVertex(last.prev,last.v.x,last.prev.v.y);
			}
			else{
				listCur.ResetVertex(last.prev,last.prev.v.x,last.v.y);
			}
		}
	  }
