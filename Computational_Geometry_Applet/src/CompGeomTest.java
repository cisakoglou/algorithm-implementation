...
public class CompGeomTest extends Frame
{
  ...
  private Checkbox xpoly, xpoint, xsegm, xchain, x2poly, xorth;
  ...
  private MenuItem area, tri, inpoly, segseg, chull,chullOrth, delTri, poly2inters, mink, oarmmove;
  ...
  public CompGeomTest(Applet a)
  {
	  ...
	  chullOrth = new MenuItem("Convex Hull Orth 2D");
      ...
      xorth = new Checkbox("OrthPol",regime,false);
  ...
  public boolean action(Event e, Object o)
  {
	...
	if (regime.getCurrent() == xpoly ) 
	  c.SetRegime("poly"); 
        else if (regime.getCurrent() == xorth)
	  c.SetRegime("orth");
	...
	if ( e.target == segseg )
	{
	  ...
	  if (regime.getCurrent() == xorth){
		if (e.target == chullOrth){
			c.ToCHorth();
		}
	  }
